...
app :: IO ()
app = Okapi.run id $ Combinators.choice [homeHandler]

data Month = Month Week Week Week Week
...
firstMonth = Month week1 week2 week3 week4
  where
    ...
    week4 =
      Week
        (Day ["PROJECT: Refactor CLI App into Web App"] "" "")
        (Day ["PROJECT: Refactor CLI App into Web App"] "" "")
        (Day ["PROJECT: Refactor CLI App into Web App"] "" "")
        (Day ["PROJECT: Refactor CLI App into Web App"] "" "")
        (Day ["PROJECT: Refactor CLI App into Web App"] "" "")
...
homeHandler = do
  Okapi.methodGET
  Okapi.pathParam @Text.Text `Okapi.equals` "" <|> Okapi.path `Okapi.equals` mempty
  Okapi.ok
    |> Okapi.setHTML html
    |> respond
  where
    ...
    calendar =
      [Perl6.qq|
            ...
            <div class="lg:flex lg:h-full lg:flex-col md:m-5 lg:m-10">
                $calendarHeader
                $calendarDays
                $dayEventList
            </div>
    ...
    htmlHead =
      [Perl6.q|
            <head>
                ...
                <title>Learn Haskell In Three Months</title>
            ...
            <header class="flex items-center justify-between border-b border-gray-200 py-4 px-6 lg:flex-none">
                ...
                <div class="flex items-center">
                    <div class="flex items-center rounded-md shadow-sm md:items-stretch">
                        ...
                        <button type="button"
                            class="hidden border-t border-b border-gray-300 bg-white px-3.5 text-sm font-medium text-gray-700 hover:bg-gray-50 hover:text-gray-900 focus:relative md:block">Module A</button>
    ...
    calendarDays :: LBS.ByteString
    calendarDays =
      [Perl6.qq|
                <div class="shadow ring-1 ring-black ring-opacity-5 lg:flex lg:flex-auto lg:flex-col">
                        {calendarDaysFull firstMonth}
                        $calendarDaysMobile
                    </div>
                </div>        
            |]

    calendarDaysSR :: LBS.ByteString
    calendarDaysSR =
      [Perl6.q|
            <div
    ...
    calendarDaysFull :: Month -> LBS.ByteString
    calendarDaysFull (Month week1 week2 week3 week4) =
      [Perl6.qq|
    ...
    calendarDaysMobile :: LBS.ByteString
    calendarDaysMobile =
      [Perl6.q|
...
respond :: Okapi.MonadOkapi m => Okapi.Response -> Okapi.Handler m
respond response = do
  Okapi.methodEnd
  Okapi.pathEnd
  pure response
